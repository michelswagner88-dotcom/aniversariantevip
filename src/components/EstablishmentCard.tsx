...
const EstablishmentCard = ({ establishment, index = 0 }: EstablishmentCardProps) => {
  ...
  const toggleFavorite = async () => {
    ...
    if (isFavorited) {
      await supabase.from("favoritos").delete().eq("usuario_id", userId).eq("estabelecimento_id", establishment.id);
      setIsFavorited(false);
    } else {
      await supabase.from("favoritos").insert({ usuario_id: userId, estabelecimento_id: establishment.id });
      setIsFavorited(true);
    }
  };
  ...
  const photo =
    establishment.galeria_fotos?.[0] ||
    establishment.logo_url ||
    establishment.photo_url ||
    "/placeholder-estabelecimento.png";
      ...
      >
        ...
        >
          ...
          <div className="relative aspect-[4/3] overflow-hidden">
            ...
            {/* BADGES ESPECIAIS (NOVO / POPULAR) - Reposicionados */}
            {(isNew || isPopular) && (
            ...
            {/* BOTÃO FAVORITAR - Touch target aumentado */}
            <button
          ...
          <div className="p-4">
            ...
            {/* Benefício - Sempre visível (mobile-friendly) */}
            {benefitSummary && (
              <div className="flex items-center gap-2 mt-3 p-2.5 bg-[#240046]/5 dark:bg-[#240046]/20 rounded-lg">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm font-semibold text-[#240046] dark:text-[#A78BFA] truncate">{benefitSummary}</p>
                </div>
              </div>
            )}
          </div>
