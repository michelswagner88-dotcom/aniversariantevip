...
import { Loader2, Navigation, TrendingUp, Map, Car } from 'lucide-react';
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {data.clicks_by_app.map((app) => {
            ...
            return (
              <div
                key={app.app_name}
                className={`p-4 rounded-lg ${colorClass} border border-white/10`}
              >
              ...
              >
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="flex-1">
                    <h4 className="text-white font-semibold mb-1">{est.nome_fantasia}</h4>
                    <div className="flex items-center gap-3 text-xs text-slate-400">
                      <span>🚗 Uber: {est.clicks_by_app.uber}</span>
                      <span>🚖 99: {est.clicks_by_app['99']}</span>
                      <span>🗺️ Waze: {est.clicks_by_app.waze}</span>
                      <span>📍 Maps: {est.clicks_by_app.maps}</span>
                    </div>
                  </div>
                </div>
