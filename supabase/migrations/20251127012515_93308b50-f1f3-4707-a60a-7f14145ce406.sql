-- Criar função RPC para upsert em massa de estabelecimentos (ignora RLS)
CREATE OR REPLACE FUNCTION public.upsert_establishment_bulk(
  p_data JSONB
)
RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  VALUES (
    (p_data->>'razao_social')::TEXT,
    (p_data->>'nome_fantasia')::TEXT,
    (p_data->>'cnpj')::TEXT,
    ARRAY(SELECT jsonb_array_elements_text(p_data->'categoria'))::TEXT[],
